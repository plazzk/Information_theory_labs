...
class CryptoApp:
    def __init__(self, window):
        ...
        self.selected_method = StringVar(value="Столбцовый метод (английский)")
    ...
    def show_column_table(self):
        ...
        table_window.title("Таблица распределения символов (столбцовый метод)")
        table_window.geometry("800x600")
        table_window.resizable(True, True)
        table_window.configure(bg=self.bg_color)

        table = self.last_table_data['table']
        key = self.last_table_data['key']
        column_order = self.last_table_data['column_order']
        filtered_text = self.last_table_data['filtered_text']
        result = self.last_table_data['result']
        mode = self.last_table_data.get('mode', 'encrypt')
        ...
        ttk.Label(info_frame, text=f"Ключ: {key}",
                  font=("Arial", 10), background=self.bg_color, foreground=self.fg_color).pack(anchor="w")
        ...
        header_frame.grid(row=0, column=0, columnspan=cols + 1, pady=5)

        ttk.Label(header_frame, text="",
                  font=("Arial", 10),
                  relief="solid", borderwidth=1,
                  width=8, anchor="center").grid(row=0, column=0, rowspan=2, padx=1, pady=1)

        for j in range(cols):
            ttk.Label(header_frame, text=f"{key[j]}",
                      font=("Arial", 12, "bold"),
                      relief="solid", borderwidth=1,
                      width=5, anchor="center").grid(row=0, column=j + 1, padx=1, pady=1)

            ttk.Label(header_frame, text=f"({column_order[j]})",
                      font=("Arial", 10),
                      relief="solid", borderwidth=1,
                      width=5, anchor="center").grid(row=1, column=j + 1, padx=1, pady=1)

        for i in range(rows):
            ttk.Label(inner_frame, text=f"Стр{i + 1}",
                      font=("Arial", 10, "bold"),
                      relief="solid", borderwidth=1,
                      width=8, anchor="center").grid(row=i + 1, column=0, padx=1, pady=1)

        for i in range(rows):
            for j in range(cols):
                cell_value = table[i][j]
                if cell_value and cell_value != '\0':
                    ttk.Label(inner_frame, text=cell_value,
                              font=("Arial", 12, "bold"),
                              relief="solid", borderwidth=1,
                              width=5, anchor="center",
                              background="#E8F5E8").grid(row=i + 1, column=j + 1, padx=1, pady=1)
                else:
                    ttk.Label(inner_frame, text="—",
                              font=("Arial", 10),
                              relief="solid", borderwidth=1,
                              width=5, anchor="center",
                              foreground="gray").grid(row=i + 1, column=j + 1, padx=1, pady=1)
    ...
    def column_encrypt(self, text, key):
        ...
        key = self.prepare_english_key(key)

        key_length = len(key)
        repeats = (len(clean_text) + key_length - 1) // key_length
        repeated_key = key * repeats
        cols = len(repeated_key)

        column_order = self.calculate_column_order(repeated_key)

        table = [list(clean_text) + [''] * (cols - len(clean_text))]

        result = []
        for order_num in range(1, cols + 1):
            col_index = column_order.index(order_num)
            if col_index < len(table[0]) and table[0][col_index]:
                result.append(table[0][col_index])
    ...
    def column_decrypt(self, text, key):
        ...
        inverse_order = [0] * cols
        for i, order_num in enumerate(column_order):
            inverse_order[order_num - 1] = i
    ...
    def filter_russian_text(self, text):
        filtered = []
        for char in text:
            upper_char = char.upper()
    ...
    def vigenere_encrypt(self, text, key):
        ...
        clean_text = self.filter_russian_text(text)
        if not clean_text:
    ...
    def vigenere_decrypt(self, text, key):
        ...
        for i in range(len(clean_text)):
            ...
            if i < len(clean_key):
                ...
                key_char = result[i - len(clean_key)]
                generated_key.append(key_char)
            ...
            step = {
                ...
                'key_char': key_char,
                'key_idx': key_idx,
                'diff': (text_idx - key_idx) % len(alphabet),
                'result_idx': result_idx,
                'result_char': result_char
            }
            steps.append(step)
        ...
        try:
            ...
            if self.selected_method.get() == "Столбцовый (англ)":
                ...
                if not filtered_text:
                    ...
                    return
                result = self.column_encrypt(filtered_text, key)
                messagebox.showinfo("Информация", f"Отфильтрованный текст: {filtered_text}")

            else:
                filtered_text = self.filter_russian_text(text)
                if not filtered_text:
                    messagebox.showwarning("Предупреждение", "Текст не содержит русских букв")
                    return
                result = self.vigenere_encrypt(filtered_text, key)
        ...
        try:
            ...
            if self.selected_method.get() == "Столбцовый (англ)":
                ...
                if not filtered_text:
                    messagebox.showwarning("Предупреждение", "Текст не содержит английских букв")
                    return
                result = self.column_decrypt(filtered_text, key)
                messagebox.showinfo("Информация", f"Отфильтрованный текст: {filtered_text}")
            else:
                filtered_text = self.filter_russian_text(text)
                if not filtered_text:
                    messagebox.showwarning("Предупреждение", "Текст не содержит русских букв")
                    return
                result = self.vigenere_decrypt(filtered_text, key)
        ...
        try:
            ...
            if self.selected_method.get() == "Столбцовый (англ)":
                ...
                if not filtered_text:
                    ...
                    return
                result = self.column_encrypt(filtered_text, key)
                result_title = "Результат шифрования (столбцовый метод)"
            else:
                filtered_text = self.filter_russian_text(text)
                if not filtered_text:
                    messagebox.showwarning("Предупреждение", "Файл не содержит русских букв")
                    return
                result = self.vigenere_encrypt(filtered_text, key)
                result_title = "Результат шифрования (Виженер)"
        ...
        try:
            ...
            if self.selected_method.get() == "Столбцовый (англ)":
                ...
                if not filtered_text:
                    ...
                    return
                result = self.column_decrypt(filtered_text, key)
                result_title = "Результат дешифрования (столбцовый метод)"
            else:
                filtered_text = self.filter_russian_text(text)
                if not filtered_text:
                    messagebox.showwarning("Предупреждение", "Файл не содержит русских букв")
                    return
                result = self.vigenere_decrypt(filtered_text, key)
                result_title = "Результат дешифрования (Виженер)"
